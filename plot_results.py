...
def plot_robustness_curve():
    ...
    scenarios = [
        {"name": "Random Failure", "file": "attack_log_random.json", "color": "green", "marker": "o"},
        {"name": "Targeted (Degree)", "file": "attack_log_degree.json", "color": "blue", "marker": "^"},
        {"name": "Targeted (Collective Influence)", "file": "attack_log_ci.json", "color": "red", "marker": "x"}
    ]
    ...
    for sc in scenarios:
        ...
        plt.plot(x, y, label=sc["name"], color=sc["color"], marker=sc["marker"], linestyle='-', linewidth=2, markersize=6)
...
def plot_efficiency_drop():
    ...
    if not os.path.exists(filepath):
        print(f"Warning: Khong tim thay log file. Dang dung du lieu gia lap...")
        x = [0.0, 0.05, 0.1, 0.15, 0.2]
    ...
    plt.plot(x, y, color='purple', marker='s', linestyle='-', linewidth=2, label='Targeted Attack (Degree)')
